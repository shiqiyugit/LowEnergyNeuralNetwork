...
def VertexMask(set_labels,azimuth_index=2,track_index=7,max_track=1.0):
    ...
    old_z_mask_start = np.logical_and(z_start > -505, z_start < 500)
    old_z_mask_end = np.logical_and(z_end>-505, z_end<500)
    ...
    vertex_mask["end_cut"] = z_mask_end
    vertex_mask["contained_DC_start"] = np.logical_and(old_z_mask_start, r_start < radius_DC)
    vertex_mask["contained_end"] = old_z_mask_end
    vertex_mask["contained_start"]= old_z_mask_start
    vertex_mask["contained_IC19_end"] = np.logical_and(old_z_mask_end, r_end < radius_IC19)
    vertex_mask["contained_IC19_start"]= np.logical_and(old_z_mask_start, r_start < radius_IC19)
