...
def IC_layers(IC_drop_value, strings_IC, input_IC):
        ...
        # -----------------------------------
def IC78_layers(IC_drop_value, input_IC, is_training):
        ...
        conv_hex3d_layers, kernels, biases = tfs.new_conv_nd_layers(
                                        ...
                                        keep_prob=IC_drop_value,
                                        **config['conv_IC78_settings']
                                        )
        layer_flat_IC78, num_features_IC78 = tfs.flatten_layer(
                                                        conv_hex3d_layers[-1])

        return layer_flat_IC78

def hex_network(X_DC,X_IC,num_labels,DC_drop_value,IC_drop_value,connected_drop_value,activation='linear',is_training=True, config=None):
	...
	dom_variables_IC = X_IC.shape[-1]

	# Conv DC + batch normalization, later dropout and maxpooling
	input_IC = Input(shape=(10, 10, dom_per_string_IC, dom_variables_IC))
	flat_IC = IC78_layers(IC_drop_value, input_IC,is_training)
