...
from keras.layers import Dropout, SpatialDropout3D
...
#    x = math_ops.cast(math_ops.greater(x, threshold), floatx())
...
def IC_layers_3D(IC_drop_value, strings_ICx, strings_ICy, input_IC):
        ...
        conv1_IC = Conv3D(32,kernel_size=(3, 3, 3),padding='same',activation='tanh')(input_IC)
        batch1_IC = BatchNormalization()(conv1_IC)
        pool1_IC = MaxPooling3D(pool_size=(1,1,2))(batch1_IC)
        drop1_IC = SpatialDropout3D(IC_drop_value)(pool1_IC)

        conv2_IC = Conv3D(32,kernel_size=(3, 3,3),padding='same',activation='relu')(drop1_IC)
        batch2_IC = BatchNormalization()(conv2_IC)
        drop2_IC = SpatialDropout3D(IC_drop_value)(batch2_IC)

        conv3_IC = Conv3D(32,kernel_size=(3,3,3),padding='same',activation='relu')(drop2_IC)
        batch3_IC = BatchNormalization()(conv3_IC)
        drop3_IC = SpatialDropout3D(IC_drop_value)(batch3_IC)

        conv4_IC = Conv3D(64,kernel_size=(3,3,3),padding='valid',activation='relu')(drop3_IC)
        batch4_IC = BatchNormalization()(conv4_IC)
        pool4_IC = MaxPooling3D(pool_size=(1,1,2))(batch4_IC)
        drop4_IC = SpatialDropout3D(IC_drop_value)(pool4_IC)

        conv5_IC = Conv3D(128,kernel_size=(1,1,7),padding='same',activation='relu')(drop4_IC)
        batch5_IC = BatchNormalization()(conv5_IC)
        drop5_IC = SpatialDropout3D(IC_drop_value)(batch5_IC)

        conv6_IC = Conv3D(128,kernel_size=(1,1,7),padding='same',activation='relu')(drop5_IC)
        batch6_IC = BatchNormalization()(conv6_IC)
        drop6_IC = SpatialDropout3D(IC_drop_value)(batch6_IC)

        conv7_IC = Conv3D(128,kernel_size=(1,1,1),padding='same',activation='relu')(drop6_IC)
        batch7_IC = BatchNormalization()(conv7_IC)
        drop7_IC = SpatialDropout3D(IC_drop_value)(batch7_IC)

        conv8_IC = Conv3D(128,kernel_size=(1,1,1),padding='same',activation='relu')(drop7_IC)
        batch8_IC = BatchNormalization()(conv8_IC)
        drop8_IC = SpatialDropout3D(IC_drop_value)(batch8_IC)

        flat_IC = Flatten()(drop8_IC)

        return flat_IC

def make_network_3D(X_DC,X_IC1, X_IC2, X_IC3, num_labels,DC_drop_value,IC_drop_value,connected_drop_value):
        ...
        # Conv DC + batch normalization, later dropout and maxpooling
        input_IC1 = Input(shape=(strings_ICx, strings_ICy, dom_per_string_IC, dom_variables_IC))
        flat_IC1 = IC_layers_3D(IC_drop_value, strings_ICx, strings_ICy, input_IC1)

        input_IC2 = Input(shape=(strings_ICx, strings_ICy, dom_per_string_IC, dom_variables_IC))
        flat_IC2 = IC_layers_3D(IC_drop_value, strings_ICx, strings_ICy, input_IC2)
        input_IC3 = Input(shape=(X_IC3.shape[1],X_IC3.shape[2], dom_per_string_IC, dom_variables_IC))
        flat_IC3 = IC_layers_3D(IC_drop_value, X_IC3.shape[1],X_IC3.shape[2], input_IC3)



        # PUT TOGETHER #
        concatted = concatenate([flat_DC, flat_IC1, flat_IC2, flat_IC3])
...
def make_network(X_DC,X_IC,num_labels,DC_drop_value,IC_drop_value,connected_drop_value,activation='linear', uncertainty=False):
...
def make_network_DC(X_DC,num_labels,DC_drop_value,IC_drop_value,connected_drop_value):
        ...
        full1 = Dense(300,activation='relu')(flat_DC)
        batch1_full = BatchNormalization()(full1)
        dropf = Dropout(connected_drop_value)(batch1_full)
        output = Dense(num_labels,activation='tanh')(dropf)
